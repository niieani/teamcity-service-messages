 ...
 * @param {String} flowId
 */
var Message = function (type, args, flowId) {
	this.type = type;
	this.single = false;
  ...
  if (!this.single && !this.args.flowId && index.autoFlowId) {
...
Message.prototype.toString = function () {
	if (this.single) {
    const template = Object.keys(this.args).length ? '##teamcity[%s \'%s\' %s]' : '##teamcity[%s \'%s\']'
		return util.format(template, this.type, this.escape(this.single), this.formatArgs());
	}
